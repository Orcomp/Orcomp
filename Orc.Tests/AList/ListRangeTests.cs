namespace Orc.DataStructures.AList
{
	...
	[TestFixture]
	public class AddRangeTest<ListT> where ListT : IGetIteratorSlice<int>, IAddRange<int>, ICloneable
	{
	...
	public class ListRangeTests<ListT> : AddRangeTest<ListT>
		where ListT : IGetIteratorSlice<int>, IListRangeMethods<int>, ICloneable
	{
		...
		public void TestRemoveRange()
		{
			...
			for (int iteration = 0; list2.Count > 0; iteration++)
			{
				...
				int amount = System.Math.Min(list2.Count - at, _r.Next(100));
		...
		public void TestIterateRange()
		{
			...
			for (int i = 0; i < 100; i++)
			{
				...
				ExpectList(list.GetIterator(at, amount),
                    Enumerable.Range(at, System.Math.Min(amount, list.Count - at)).ToArray());
			}
		}
		...
		protected virtual int StressTestIteration(ref ListT list, List<int> list2, int i, int at, int amount)
		{
			...
			{
                int amount2 = System.Math.Min(amount, list.Count - at);
				ExpectList(list.GetIterator(at, amount),
