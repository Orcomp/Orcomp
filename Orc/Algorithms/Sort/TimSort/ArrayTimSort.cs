...
namespace Orc.Algorithms.Sort.TimSort
{
    using System;
	...
	internal class ArrayTimSort<T>
	{
		...
		private ArrayTimSort(T[] array, Comparison<T> comparer)
		{
			...
			this.m_MergeBuffer = new T[mergeBufferLength];
			...
			int stackLength =
				...
				40;
			this.m_RunBase = new int[stackLength];
			this.m_RunLength = new int[stackLength];
		}
		...
		{
			this.m_RunBase[this.m_StackSize] = runBase;
			this.m_RunLength[this.m_StackSize] = runLength;
			this.m_StackSize++;
		}
		...
		{
			while (this.m_StackSize > 1)
			{
				int n = this.m_StackSize - 2;

				if (n > 0 && this.m_RunLength[n - 1] <= this.m_RunLength[n] + this.m_RunLength[n + 1])
				{
					if (this.m_RunLength[n - 1] < this.m_RunLength[n + 1]) n--;
					this.MergeAt(n);
				}
				else if (this.m_RunLength[n] <= this.m_RunLength[n + 1])
				{
					this.MergeAt(n);
				}
				else
		...
		private void MergeForceCollapse()
		{
			...
			{
				int n = this.m_StackSize - 2;
				if (n > 0 && this.m_RunLength[n - 1] < this.m_RunLength[n + 1]) n--;
				this.MergeAt(n);
			}
		}
		...
		{
			Debug.Assert(this.m_StackSize >= 2);
			Debug.Assert(runIndex >= 0);
			Debug.Assert(runIndex == this.m_StackSize - 2 || runIndex == this.m_StackSize - 3);

			int base1 = this.m_RunBase[runIndex];
			int len1 = this.m_RunLength[runIndex];
			int base2 = this.m_RunBase[runIndex + 1];
			int len2 = this.m_RunLength[runIndex + 1];
			...
			this.m_RunLength[runIndex] = len1 + len2;
			if (runIndex == this.m_StackSize - 3)
			{
				this.m_RunBase[runIndex + 1] = this.m_RunBase[runIndex + 2];
				this.m_RunLength[runIndex + 1] = this.m_RunLength[runIndex + 2];
			}
			this.m_StackSize--;

			// Find where the first element of run2 goes in run1. Prior elements
			// in run1 can be ignored (because they're already in place).
			int k = GallopRight(this.m_Array[base2], this.m_Array, base1, len1, 0, this.m_Comparer);
			Debug.Assert(k >= 0);
			base1 += k;
			len1 -= k;
			if (len1 == 0) return;
			...
			len2 = GallopLeft(this.m_Array[base1 + len1 - 1], this.m_Array, base2, len2, len2 - 1, this.m_Comparer);
			Debug.Assert(len2 >= 0);
			if (len2 == 0) return;
		...
		private void MergeLo(int base1, int len1, int base2, int len2)
		{
			...
			var array = this.m_Array; // For performance
			var mergeBuffer = this.EnsureCapacity(len1);
			...
			var c = this.m_Comparer;  // Use local variables for performance
			int minGallop = this.m_MinGallop;
			...
			this.m_MinGallop = minGallop < 1 ? 1 : minGallop;  // Write back to field
		...
		private void MergeHi(int base1, int len1, int base2, int len2)
		{
			...
			var a = this.m_Array; // For performance
			var tmp = this.EnsureCapacity(len2);
		...
		{
			if (this.m_MergeBuffer.Length < minCapacity)
			{
				...
				if (newSize < 0) // Not bloody likely!
					...
					newSize = Math.Min(newSize, this.m_Array.Length >> 1);

				this.m_MergeBuffer = new T[newSize];
			}
