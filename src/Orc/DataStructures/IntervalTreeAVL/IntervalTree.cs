...
namespace Orc.DataStructures.IntervalTreeAVL
{
    using System;
    ...
    public class IntervalTree<T> : IIntervalContainer<T> where T : struct, IComparable<T>
    {
        ...
        public void Remove(IInterval<T> interval)
        {
            ...
            this.Delete(interval as Interval<T>);
        }
        ...
        public IEnumerable<IInterval<T>> Query(IInterval<T> interval)
        {
            ...
            var results = this.GetIntervalsOverlappingWith(interval as Interval<T>);
        ...
        private bool TryGetIntervalImpl(IntervalNode subtree, Interval<T> data, out  IInterval<T> value)
        {
            if (subtree != null)
            {
                int compareResult = data.Min.Value.CompareTo(subtree.Interval.Min.Value);
                ...
                {
                    if (data.Max.Value.CompareTo(subtree.Interval.Max.Value) == 0)
                    {
                        value = subtree.Value;
                        return true;
                    }
        ...
        private class IntervalNode
        {
            ...
            public static IntervalNode Add(IntervalNode elem, Interval<T> interval, ref bool wasAdded, ref bool wasSuccessful)
            {
                ...
                {
                    int compareResult = interval.Min.Value.CompareTo(elem.Interval.Min.Value);
            ...
            {
                T maxRange = node.Interval.Max.Value;
            ...
            /// <summary>
            /// Gets the range of intervals stored in this.Range (i.e. starting at the same value 'this.Interval.Min.Value' as the interval stored in the node itself)
            ...
            {
                int cmp = arg.Min.Value.CompareTo(node.Interval.Min.Value);
                IntervalNode newChild = null;
                ...
                {
                    if (arg.Max.Value.CompareTo(node.Interval.Max.Value) == 0 && node.Range == null)
                    {
                        if (node.Left != null && node.Right != null)
                        {
            ...
            public static List<KeyValuePair<Interval<T>, IInterval<T>>> GetIntervalsStartingAt(IntervalNode subtree, T start)
            {
                if (subtree != null)
                {
                    int compareResult = start.CompareTo(subtree.Interval.Min.Value);
                    if (compareResult < 0)
                    {
            ...
            public void GetIntervalsOverlappingWith(Interval<T> toFind, ref List<KeyValuePair<Interval<T>, IInterval<T>>> list)
            {
                ...
                else if (toFind.Min.Value.CompareTo(this.Max) >= 0)
                {
            ...
            private bool DeleteIntervalFromNodeWithRange(Interval<T> interval)
            {
                if (this.Range != null && this.Range.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (interval.Max.Value.CompareTo(this.Range[k].Key) == 0)
                            {
                                intervalPosition = k + 1;
                                break;
                            }
                    ...
                    {
                        this.Interval = new Interval<T>(this.Interval.Min.Value, this.Range[0].Key);
